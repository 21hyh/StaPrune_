...
class Qwen2_5_VLVisionFlashAttention2(nn.Module):
    ...
    def forward(
        ...
        # Some flash-attn / transformers combinations return auxiliary metadata
        # alongside the attention output. Only the first tensor is the residual.
...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        if return_attn_weights:
            ...
            attn_weights = attn_weights.masked_fill(~attention_mask, torch.finfo(q.dtype).min)
            attn_weights = nn.functional.softmax(attn_weights, dim=-1)
...
class Qwen2_5_VLVisionBlock(nn.Module):
    ...
    def forward(
        ...
        if isinstance(attn, tuple):
            if attn_weights is None and len(attn) > 1:
                attn_weights = attn[1]
            attn = attn[0]
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        # StaPrune parameters
        self.tok_keep_ratio = kwargs.get('tok_keep_ratio', None)
        self.prune_method = kwargs.get('prune_method', 'prunemerge')
        self.selection_strategy = kwargs.get('selection_strategy', 'auto')
        self.select_layer = kwargs.get('select_layer', -2)
    ...
    def _resolve_select_layer_block_index(self):
        total_hidden_states = len(self.blocks) + 1
        resolved_layer = self.select_layer if self.select_layer >= 0 else total_hidden_states + self.select_layer
        if resolved_layer <= 0 or resolved_layer >= total_hidden_states:
            raise ValueError(
                f"Invalid select_layer={self.select_layer} for vision depth={len(self.blocks)}"
            )
        return resolved_layer - 1
    ...
    def _select_and_merge_tokens_v105(self, patch_features, importance_scores, selected_indices, k_total, D, device, dtype):
        ...
        B, N, _ = patch_features.shape
        selected_features = torch.gather(patch_features, 1, selected_indices.unsqueeze(-1).expand(-1, -1, D))
    ...
    def _v105_selection(self, patch_features, attn_scores, N, D, device, dtype, original_seq_len=None):
        """V105: simple two-stage selection with optional token merge."""
        # 基于原始token数量计算保留的patch数量
        if original_seq_len is not None:
            k_total_tokens = math.ceil(original_seq_len * self.tok_keep_ratio)
            k_total = max(1, k_total_tokens // self.spatial_merge_unit)
        else:
            k_total = math.ceil(N * self.tok_keep_ratio)
        hybrid_scores = attn_scores / (attn_scores.sum(dim=-1, keepdim=True) + 1e-8)
        if self.tok_keep_ratio >= 0.25:
        ...
        direct_indices = torch.topk(hybrid_scores, k=k_direct, dim=-1).indices
        final_indices = self._greedy_max_min_diversity_v105(patch_features, hybrid_scores, direct_indices, k_diverse, device)
        if self.tok_keep_ratio < 0.15:
            selected_features = self._select_and_merge_tokens_v105(patch_features, hybrid_scores, final_indices, k_total, D, device, dtype)
        else:
            topk_sorted = torch.sort(final_indices, dim=-1).values
            selected_features = torch.gather(patch_features, 1, topk_sorted.unsqueeze(-1).expand(-1, -1, D))
        return selected_features

    def _v139_post_selection_refinement(self, patch_features, attn_scores, N, grid_h, grid_w, D, device, dtype, original_seq_len=None):
        ...
        for b in range(B):
            ...
            stage1_indices = torch.topk(attention, k=k_stage1).indices
            ...
            remaining_mask[selected_so_far] = False
            remaining_indices = all_indices[remaining_mask]
            stage3_indices = self._max_min_diversity_selection(patch_features_norm[b], selected_so_far, remaining_indices, k_stage3, device)
            final_indices = torch.cat([stage1_indices, stage2_indices, stage3_indices])
            num_regions = 4
            region_h_size = math.ceil(grid_h / num_regions)
            region_w_size = math.ceil(grid_w / num_regions)
            coverage_map = torch.zeros(num_regions, num_regions, device=device)
            for idx in final_indices:
                h = (idx // grid_w).item()
                w = (idx % grid_w).item()
                region_h = min(num_regions - 1, h // region_h_size)
                region_w = min(num_regions - 1, w // region_w_size)
            ...
            if weak_regions:
                ...
                for s3_idx in stage3_indices:
                    s3_feat = patch_features_norm[b][s3_idx]
                    similarities = torch.mv(selected_features, s3_feat)
                    min_sim = similarities.min().item()
                    stage3_marginality.append((s3_idx.item(), min_sim))
                ...
                for region_h, region_w in weak_regions[:max_swaps]:
                    h_start = region_h * region_h_size
                    h_end = min(grid_h, h_start + region_h_size)
                    w_start = region_w * region_w_size
                    w_end = min(grid_w, w_start + region_w_size)
                    region_candidates = []
                    ...
                    if region_candidates:
                        ...
                        region_attention = attention[region_candidates_tensor]
                        best_candidate = region_candidates_tensor[torch.argmax(region_attention)]
            ...
            selected_features = torch.gather(patch_features[b], 0, final_indices_sorted.unsqueeze(-1).expand(-1, D))
            selected_features_list.append(selected_features)
        return torch.stack(selected_features_list, dim=0)

    def _v112_retention_adaptive(self, patch_features, attn_scores, N, grid_h, grid_w, D, device, dtype, original_seq_len=None):
        ...
        if keep_ratio >= 0.25:
            ...
            return self._v139_post_selection_refinement(patch_features, attn_scores, N, grid_h, grid_w, D, device, dtype, original_seq_len)
        if not hasattr(self, "_printed_delegate_v105"):
            print(f"\n[Auto Router] Retention: {keep_ratio:.1%} ({k_total}/{N} tokens)")
            print("  Strategy: Delegate to V105")
            self._printed_delegate_v105 = True
        return self._v105_selection(patch_features, attn_scores, N, D, device, dtype, original_seq_len)

    def _v105_select_groups(self, image_groups, image_group_features, image_attn_scores, N, device):
        D = image_group_features.shape[-1]
        patch_features = image_group_features.unsqueeze(0)
        hybrid_scores = image_attn_scores.unsqueeze(0)
        ...
        final_indices_sorted = torch.sort(final_indices).values
        selected_groups = image_groups[final_indices_sorted]

        if self.tok_keep_ratio < 0.15:
            k_merge_candidates = min(k_total, N - k_total)
            if k_merge_candidates > 0:
                _, top_indices = torch.topk(hybrid_scores, k=min(k_total * 2, N), dim=-1)
                selected_set = set(final_indices.tolist())
                unselected = [idx.item() for idx in top_indices[0] if idx.item() not in selected_set]
                unselected = unselected[:k_merge_candidates]
                if unselected:
                    unselected_groups = image_groups[unselected]
                    unselected_importance = hybrid_scores[0, unselected]
                    weights = unselected_importance / (unselected_importance.sum() + 1e-8)
                    merged_group = (unselected_groups * weights.view(-1, 1, 1)).sum(dim=0, keepdim=True)
                    selected_groups = torch.cat([selected_groups, merged_group], dim=0)

        return selected_groups

    def _v139_select_groups(self, image_groups, image_group_features, image_attn_scores, N, grid_h, grid_w, device):
        k_total = math.ceil(N * self.tok_keep_ratio)
        k_stage1 = int(k_total * 0.27)
        k_stage2 = int(k_total * 0.46)
        k_stage3 = k_total - k_stage1 - k_stage2
        patch_features_norm = image_group_features / (image_group_features.norm(dim=-1, keepdim=True) + 1e-8)

        attention = image_attn_scores
        stage1_indices = torch.topk(attention, k=k_stage1).indices
        ...
        stage3_indices = self._max_min_diversity_selection(
            patch_features_norm, selected_so_far, remaining_indices, k_stage3, device
        )
        ...
        if weak_regions:
            max_swaps = min(5, len(weak_regions))
            stage3_marginality = []
            selected_features = patch_features_norm[final_indices]
            for s3_idx in stage3_indices:
                s3_feat = patch_features_norm[s3_idx]
                similarities = torch.mv(selected_features, s3_feat)
                min_sim = similarities.min().item()
                stage3_marginality.append((s3_idx.item(), min_sim))
            stage3_marginality.sort(key=lambda x: x[1])
        ...
        final_indices_sorted = torch.sort(final_indices).values
        return image_groups[final_indices_sorted]

    def _prune_grouped_image_tokens(self, grouped_hidden_states, selection_grouped_hidden_states, group_attn_scores, grid_thw):
        if grid_thw is None or grid_thw.numel() == 0 or torch.any(grid_thw[:, 0] != 1):
            return self.merger(grouped_hidden_states)

        device = grouped_hidden_states.device
        group_lengths = (
            grid_thw[:, 0]
        ...
        grouped_hidden_splits = torch.split(grouped_hidden_states, group_lengths, dim=0)
        selection_grouped_splits = torch.split(selection_grouped_hidden_states, group_lengths, dim=0)
        attn_score_splits = torch.split(group_attn_scores, group_lengths, dim=0)
        selected_outputs = []

        for image_groups, selection_image_groups, image_attn_scores, image_grid in zip(
            grouped_hidden_splits, selection_grouped_splits, attn_score_splits, grid_thw
        ):
            llm_grid_h = image_grid[1].item() // self.spatial_merge_size
            llm_grid_w = image_grid[2].item() // self.spatial_merge_size
            N = image_groups.shape[0]
            if N != llm_grid_h * llm_grid_w:
                raise ValueError(f"Image token count mismatch: tokens={N}, grid={llm_grid_h}x{llm_grid_w}")

            image_group_features = selection_image_groups.mean(dim=1)
            if self.selection_strategy in ["auto", "v112"]:
                if self.tok_keep_ratio >= 0.25:
                    selected_groups = self._v139_select_groups(
                        image_groups, image_group_features, image_attn_scores, N, llm_grid_h, llm_grid_w, device
                    )
                else:
                    selected_groups = self._v105_select_groups(
                        image_groups, image_group_features, image_attn_scores, N, device
                    )
            ...
            selected_outputs.append(self.merger(selected_groups))

        return torch.cat(selected_outputs, dim=0)

    def _prune_merged_image_tokens(self, merged_hidden_states, attn_scores, grid_thw):
        if grid_thw is None or grid_thw.numel() == 0 or torch.any(grid_thw[:, 0] != 1):
            return merged_hidden_states

        device = merged_hidden_states.device
        dtype = merged_hidden_states.dtype
        D = merged_hidden_states.shape[-1]
        image_lengths = (
            grid_thw[:, 0]
            * (grid_thw[:, 1] // self.spatial_merge_size)
            * (grid_thw[:, 2] // self.spatial_merge_size)
        ...
        hidden_state_splits = torch.split(merged_hidden_states, image_lengths, dim=0)
        attn_score_splits = torch.split(attn_scores, image_lengths, dim=0)
        selected_features_list = []

        for image_hidden_states, image_attn_scores, image_grid in zip(hidden_state_splits, attn_score_splits, grid_thw):
            llm_grid_h = image_grid[1].item() // self.spatial_merge_size
            llm_grid_w = image_grid[2].item() // self.spatial_merge_size
            ...
            patch_features = image_hidden_states.unsqueeze(0)
            image_attn_scores = image_attn_scores.unsqueeze(0)

            if self.selection_strategy in ["auto", "v112"]:
                selected_features = self._v112_retention_adaptive(
                    patch_features, image_attn_scores, N, llm_grid_h, llm_grid_w, D, device, dtype
                )
            elif self.selection_strategy == "v139g":
                ...
                selected_features = self._v105_selection(
                    patch_features, image_attn_scores, N, D, device, dtype
                )
            else:
                raise ValueError(f"Unknown selection_strategy: {self.selection_strategy}")

            selected_features_list.append(selected_features.squeeze(0))

        return torch.cat(selected_features_list, dim=0)
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        attn_list = []
        last_fullatt_attn = None
        select_layer_hidden_states = None
        select_layer_block_idx = self._resolve_select_layer_block_index()
        need_pruning_attn = self.tok_keep_ratio is not None and self.tok_keep_ratio < 1.0
        for layer_num, blk in enumerate(self.blocks):
            if layer_num in self.fullatt_block_indexes:
                cu_seqlens_now = cu_seqlens
            else:
                cu_seqlens_now = cu_window_seqlens
            return_attn_weights = need_pruning_attn and layer_num == self.fullatt_block_indexes[-1]
            if self.gradient_checkpointing and self.training:
                hidden_states = self._gradient_checkpointing_func(
                    blk.__call__, hidden_states, cu_seqlens_now, None, position_embeddings, return_attn_weights
                )
                attn_weights = None
            else:
                ...
                if return_attn_weights and attn_weights is not None:
                    with torch.no_grad():
                        last_fullatt_attn = attn_weights.mean(dim=0).mean(dim=0)

            if layer_num == select_layer_block_idx:
                select_layer_hidden_states = hidden_states

        grouped_hidden_states = hidden_states.reshape(seq_len // self.spatial_merge_unit, self.spatial_merge_unit, -1)
        grouped_hidden_states = grouped_hidden_states[reverse_indices]

        if self.tok_keep_ratio is not None and self.tok_keep_ratio < 1.0 and last_fullatt_attn is not None:
            if select_layer_hidden_states is None:
                raise ValueError(f"Failed to capture select_layer={self.select_layer} hidden states")
            selection_grouped_hidden_states = select_layer_hidden_states.reshape(
                seq_len // self.spatial_merge_unit, self.spatial_merge_unit, -1
            )
            selection_grouped_hidden_states = selection_grouped_hidden_states[reverse_indices]
            group_attn_scores = last_fullatt_attn.view(last_fullatt_attn.shape[0] // self.spatial_merge_unit, -1).mean(dim=-1)
            group_attn_scores = group_attn_scores[reverse_indices]
            hidden_states = self._prune_grouped_image_tokens(
                grouped_hidden_states, selection_grouped_hidden_states, group_attn_scores, grid_thw
            )
        else:
            hidden_states = self.merger(grouped_hidden_states)
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, **kwargs):
        ...
        # Pass pruning parameters to vision encoder
        self.visual = Qwen2_5_VisionTransformerPretrainedModel(
            config.vision_config,
            tok_keep_ratio=self.tok_keep_ratio,
    ...
    def forward(
        ...
        image_grid_thw: Optional[torch.LongTensor] = None,
        vision_image_grid_thw: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
        second_per_grid_ts: Optional[torch.Tensor] = None,
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_embeds, attn_list = self.visual(
                    pixel_values,
                    grid_thw=vision_image_grid_thw if vision_image_grid_thw is not None else image_grid_thw,
                )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        model_inputs["vision_image_grid_thw"] = vision_image_grid_thw
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        visual_keys = ["pixel_values", "image_grid_thw", "vision_image_grid_thw", "pixel_values_videos", "video_grid_thw", "second_per_grid_ts"]

        def _expand_dict_for_generation_visual(dict_to_expand):
            image_grid_thw = model_kwargs.get("image_grid_thw", None)
            vision_image_grid_thw = model_kwargs.get("vision_image_grid_thw", image_grid_thw)
            ...
            for key in dict_to_expand:
                if key == "pixel_values":
                    ...
                    samples = torch.split(vision_image_grid_thw, list(image_nums))
                ...
                elif key == "vision_image_grid_thw":
                    lengths = list(image_nums)
                    dict_to_expand[key] = _repeat_interleave_samples(
                        dict_to_expand[key], lengths=lengths, repeat_times=expand_size
                    )
